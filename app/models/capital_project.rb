class CapitalProject < ActiveRecord::Base
    ...
    def self.capital_project_report_to_csv(options = {})
        selected_columns = $selected_array_of_headers
        column_names = selected_columns
        array_of_values = $selected_array_of_values
        CSV.generate(options) do |csv|
            csv << column_names
            all.each do |_capital_project|
              csv << array_of_values.map { |n| eval n }
            end
        end
    end

    def self.generate_report(selected_project_columns, _department_id, subdepartment_id,
                             idp_number, vote_number,
                             project_name, project_description,
                             mscore_classification_id, _planned_start_date,
                             _planned_completion_date, funding_source,
                             ward_id,
                             area_id, mun_cp_ref, _actual_start_date,
                             _actual_completion_date)
        ...
        if selected_project_columns.include?('Project Description')
            ...
            unless project_description.blank?
