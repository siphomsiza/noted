...
RSpec.describe UsersController, :type => :controller do
  ...
  describe "#show" do
    ...
    context "user is logged on to the system" do
      xit {response[0].must_equal 200}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("show")}
    end
  ...
  describe "#edit new user" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("edit_new_user")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#edit active user" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("edit_active_user")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#edit user profile" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("edit_user_profile")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#update" do
    context "when user is logged in" do
          ...
          xit {expect(response.status).to eq(200) }
          xit {expect(response.content_type).to eq("text/html") }
          xit {flash[:success].should eq("user was successfully updated.")}
          xit {expect(response).to redirect_to("/users")}
        end
  end
  describe "#deactivate" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
     xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("/:id/deactivate")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#set admin" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("/users")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
     xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#restore" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("restore")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#lock user" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("lock_user")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "#unlock user" do
    ...
    context "user is correct user or user is admin and is logged on to the system" do
      xit {expect(response).to have_http_status(200)}
      xit {expect(response.content_type).to eq("text/html") }
      xit {expect(response).to render_template("unlock_user")}
    end
    context "user is not logged on to the system" do
      xit {expect(response).to redirect_to(login_path)}
      xit {flash[:danger].should eq("Please log in.")}
  ...
  describe "Get destroy" do
    context "when user is logged in" do
          ...
          xit {assigns(:user).to eq(@user)}
          xit {expect(response.status).to eq(200) }
          it {expect(response.content_type).to eq("text/html") }
          xit {flash[:success].should eq("User deleted.")}
          it {expect(response).to redirect_to("/users")}
        end
  end
end
