...
RSpec.describe SessionsController, :type => :controller do
  ...
  context 'when password is invalid' do
    it 'renders the page with error' do
      ...
      expect(response).to render_template(:new)
      expect(flash[:danger]).to eq("wrong log on information provided...")
    end
  end
  ...
  context "User is not activated" do
    ...
    context "User is terminated" do
          ...
          expect(response).to have_http_status(200)
          expect(flash[:danger]).to eq('Account terminated.please contact your system administrator.')
    ...
    context "User is locked" do
          ...
          expect { post :create, { :email=> @user.email, :password=>"ghgh576fg55654" } }
          expect(response).to have_http_status(200)
          expect(flash[:danger]).to eq('Account Locked.please contact your system administrator.')
          expect(response).to redirect_to(root_url)
          end
    end
