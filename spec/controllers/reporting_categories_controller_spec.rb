...
RSpec.describe ReportingCategoriesController, :type => :controller do
	...
	describe "GET index" do
      context "when user is admin and logged in" do
            ...
            before do
               reporting_category = ReportingCategory.create
              @user = create(:user)
              session[:user_id] = @user.id
              get :index
            end

            it "assigns @reporting_category" do
            reporting_categories = ReportingCategory.all
            expect(reporting_categories).to be_truthy
            end
  ...
  describe 'POST #create' do
    ...
    context 'with valid attributes' do
      it 'creates the reporting_category' do
        ...
        expect(ReportingCategory.count).to eq(1)
      end
  ...
  describe "#update" do
          ...
          expect(flash[:success]).not_to eq("Reporting Category was not updated.")
          expect(response).to redirect_to :back
          end
  end
